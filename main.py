    ...
    if (
        ...
        for metric in inboundPayload.metrics:
            if (
                ...
                print(
                    "CMD message for output/Device Metric2 - New Value: {}".format(
                        newValue
                    )
                )
...
def publishDeviceBirth():
    ...
    client.publish(
        "spBv1.0/" + myGroupId + "/DBIRTH/" + myNodeName + "/" + myDeviceName,
        totalByteArray,
        0,
        False,
    )
...
client.will_set(
    "spBv1.0/" + myGroupId + "/NDEATH/" + myNodeName, deathByteArray, 0, False
)
...
while True:
    ...
    byteArray = bytearray(payload.SerializeToString())
    client.publish(
        "spBv1.0/" + myGroupId + "/DDATA/" + myNodeName + "/" + myDeviceName,
        byteArray,
        0,
        False,
    )
